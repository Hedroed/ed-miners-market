...
class CommodityMaxPrice(Base):
    ...
    def __repr__(self):
        return "<CommodityMAXPrice(commodity_id='%s', system='%s', station='%s', date='%s', market=%s)>" % (self.commodity_id, self.sell_price, self.sell_demand, self.timestamp, self.market)
